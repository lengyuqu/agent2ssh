...
### 0. UI v3 改版：午夜指挥舱（2026-08-28，已完成）

- 产品定位修正：SSH 配置管理中心 + AI 命令审查台（非标准终端工具）。设计定稿见根目录 `UI_DESIGN_PROPOSAL.md`，施工图见 `docs/plans/ui-v3-midnight-ops.md`。
- T01–T05 全量落地（commits 710c37a..94c09e4，两轮 QA 均 PASS、0 源码缺陷，73/73 前端测试）：Token 换血（指挥舱设为默认主题，旧 5 主题选择器保留兼容）、审批工作台三栏主界面（默认落点 approvals）、TopBar 状态顶栏、主机策略摘要行 + 审计风险 chips、⌘K 五分组、Toast 堆叠规范、TerminalDrawer 终端抽屉 + 审计回放。
- **取代关系**（下文历史条目保留作档案，不再反映当前实现）：`Footbar.tsx` 已删除 → 状态并入 `TopBar.tsx`；`ApprovalTimeline.tsx` 已删除 → 演进为 `src/components/approval/`（ApprovalWorkbench/ApprovalQueue/ApprovalDetail）。
- 遗留待后端（任务单见 `docs/plans/ui-v3-backend-fields.md`）：审批 Agent 来源 / matched_rule / 主机「仅人工」标记 / 审计裁决结果字段；批准后抽屉展示 exec 输出；「编辑后执行」真改写。桌面 GUI 目检（`npm run tauri:dev`）待人工执行。
...
| V1-2 | ✅ 已完成 | P0 | 低 | 全局状态栏 Footbar | `src/components/Footbar.tsx`：底部固定栏显示 daemon 状态(green/red)、gate 状态、凭据(locked/unlocked)、活跃连接数、版本号（从 `package.json` 读取）；`npm run build` 通过，Playwright 截图验证 light/dark 主题下渲染正常，各状态字段随 mock 数据正确切换（⚠️ 已被 UI v3 改版取代：`Footbar.tsx` 已删除，状态并入 `TopBar.tsx`，见「UI v3 改版」章节） |
...
V4 发布候选额外验证桌面端视觉一致性：macOS `.app` 启动后逐一检查暗色/亮色模式、Footbar、命令面板、面包屑在各模块的渲染稳定性。（⚠️ UI v3 改版后此处为 TopBar 状态顶栏，默认主题为指挥舱 midnight-ops）
