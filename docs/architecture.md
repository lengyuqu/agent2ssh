...
| `src-tauri/src/bin/agent2ssh-mcp.rs` | MCP stdio server (JSON-RPC 2.0, 58 tools) |
...
The desktop app, CLI, MCP server, and daemon share the same Rust core library. The MCP server exposes 58 tools and speaks JSON-RPC 2.0 over stdio, making it compatible with any MCP-capable agent host.
