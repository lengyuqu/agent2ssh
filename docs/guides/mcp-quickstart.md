...
Agent2SSH 通过 MCP (Model Context Protocol) stdio 协议暴露 **71 个工具**，使任何兼容 MCP 的 AI Agent 都能直接管理 SSH 主机、命令片段、执行远程命令、传输文件、管理会话、端口转发、Playbook、审计、健康检查、指标、远程 daemon，以及脱敏/高亮/算法三类本地配置。
...
配置完成后，Agent 将自动发现并调用所有 71 个 SSH 工具。
...
完整 71 个工具列表与权威描述以 [MCP Tools Reference](../skills.md) 为准。下表只列出最常用的基础入口，编号对应 `tools/list` 返回顺序的前 35 个。
